/***************************************************
  Leak Sensor based on NodeMCU

  The sensor will send a MQTT Message TBD
 ****************************************************/
#include <Arduino.h>
#include <WiFi.h>
...
void check_status_leak(){
  ...
  if(status_leak != previous_status_leak)
  {
    ...
    if(status_leak){digitalWrite(LED_STATUS_PIN,HIGH);}
    else{digitalWrite(LED_STATUS_PIN,LOW);}

    if(status_leak){publishMsg(leak_status,"No Leak");}
    else{publishMsg(leak_status,"Leak");}
  }
}
...
void loop() {
  ...
  if(currentTimer0 - previousTimer0 > interval0){
    ...
    if(debug){Serial.println(F("Timer 0"));}
  ...
  if(currentTimer1 - previousTimer1 > interval1){
    ...
    if(debug){Serial.println(F("Timer 1"));}
